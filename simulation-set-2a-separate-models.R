...
crt_updated_APR29_X4_con_SEP <- matrix(NA, nrow = 243, ncol = 28)
...
for (j in 1:length(cluster_var)){
  ...
  for (m in 1:n_sims){
    ...
    gee_true_trt <- tryCatch(geeglm(outcomes ~ race + proportion + exp(continuous_pred) + cont_ave, id = cluster, data = full_data_trt, family = gaussian, corstr = "exchangeable"), error = function(e) NULL)
  ...
  crt_updated_APR29_X4_con_SEP[index, 1] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,2] - 1.96*treatment_diff[,3]) & 
                                                treatment_diff[,1] <= (treatment_diff[,2] + 1.96*treatment_diff[,3]), 1, 0)) / nrow(treatment_diff)
  
  crt_updated_APR29_X4_con_SEP[index, 2] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,6] - 1.96*treatment_diff[,7]) & 
                                                treatment_diff[,1] <= (treatment_diff[,6] + 1.96*treatment_diff[,7]), 1, 0)) / nrow(treatment_diff)
  
  crt_updated_APR29_X4_con_SEP[index, 3] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,4] - 1.96*treatment_diff[,5]) & 
                                                treatment_diff[,1] <= (treatment_diff[,4] + 1.96*treatment_diff[,5]), 1, 0)) / nrow(treatment_diff)
  
  crt_updated_APR29_X4_con_SEP[index, 4] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,8] - 1.96*treatment_diff[,9]) & 
                                                treatment_diff[,1] <= (treatment_diff[,8] + 1.96*treatment_diff[,9]), 1, 0)) / nrow(treatment_diff)
  
  crt_updated_APR29_X4_con_SEP[index, 5] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,10] - 1.96*treatment_diff[,11]) & 
                                                treatment_diff[,1] <= (treatment_diff[,10] + 1.96*treatment_diff[,11]), 1, 0)) / nrow(treatment_diff)
  
  crt_updated_APR29_X4_con_SEP[index, 6] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,12] - 1.96*treatment_diff[,13]) & 
                                                treatment_diff[,1] <= (treatment_diff[,12] + 1.96*treatment_diff[,13]), 1, 0)) / nrow(treatment_diff)
  
  crt_updated_APR29_X4_con_SEP[index, 7] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,14] - 1.96*treatment_diff[,15]) & 
                                                treatment_diff[,1] <= (treatment_diff[,14] + 1.96*treatment_diff[,15]), 1, 0)) / nrow(treatment_diff)
  
  # MSE of every model
  crt_updated_APR29_X4_con_SEP[index, 8] <- (sum(((treatment_diff[,2] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_updated_APR29_X4_con_SEP[index, 9] <- (sum(((treatment_diff[,6] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_updated_APR29_X4_con_SEP[index, 10] <- (sum(((treatment_diff[,4] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_updated_APR29_X4_con_SEP[index, 11] <- (sum(((treatment_diff[,8] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_updated_APR29_X4_con_SEP[index, 12] <- (sum(((treatment_diff[,10] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_updated_APR29_X4_con_SEP[index, 13] <- (sum(((treatment_diff[,12] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_updated_APR29_X4_con_SEP[index, 14] <- (sum(((treatment_diff[,14] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  # summed absolute bias of every model
  crt_updated_APR29_X4_con_SEP[index, 15] <- mean(abs(treatment_diff[,1] - treatment_diff[,2]))
  
  crt_updated_APR29_X4_con_SEP[index, 16] <- mean(abs(treatment_diff[,1] - treatment_diff[,6]))
  
  crt_updated_APR29_X4_con_SEP[index, 17] <- mean(abs(treatment_diff[,1] - treatment_diff[,4]))
  
  crt_updated_APR29_X4_con_SEP[index, 18] <- mean(abs(treatment_diff[,1] - treatment_diff[,8]))
  
  crt_updated_APR29_X4_con_SEP[index, 19] <- mean(abs(treatment_diff[,1] - treatment_diff[,10]))
  
  crt_updated_APR29_X4_con_SEP[index, 20] <- mean(abs(treatment_diff[,1] - treatment_diff[,12]))
  
  crt_updated_APR29_X4_con_SEP[index, 21] <- mean(abs(treatment_diff[,1] - treatment_diff[,14]))
  
  # Average Interval width
  
  crt_updated_APR29_X4_con_SEP[index, 22] <- mean(2*1.96*treatment_diff[,3])
  
  crt_updated_APR29_X4_con_SEP[index, 23] <- mean(2*1.96*treatment_diff[,7])
  
  crt_updated_APR29_X4_con_SEP[index, 24] <- mean(2*1.96*treatment_diff[,5])
  
  crt_updated_APR29_X4_con_SEP[index, 25] <- mean(2*1.96*treatment_diff[,9])
  
  crt_updated_APR29_X4_con_SEP[index, 26] <- mean(2*1.96*treatment_diff[,11])
  
  crt_updated_APR29_X4_con_SEP[index, 27] <- mean(2*1.96*treatment_diff[,13])
  
  crt_updated_APR29_X4_con_SEP[index, 28] <- mean(2*1.96*treatment_diff[,15])
  ...
  save(crt_updated_APR29_X4_con_SEP, file = "crt_updated_APR29_X4_con_SEP.Rdata")
