...
crt_updated_OCT28_1_X4_con_DIFFTRT_SEP <- matrix(NA, nrow = 243, ncol = 28)
...
seeds <- (1:(length(cluster_var)*n_sims*3) + (length(cluster_var)*n_sims*3)*27)
...
for (j in 1:length(cluster_var)){
  ...
  for (m in 1:n_sims){
    ...
    # random mean for each cluster diff between trt and ctrl
    cluster_mean <- matrix(NA, nrow = n_clusters[j], ncol = 1)
    for (g in 1:length(treated)){
      if (treated[g] == 0){
        cluster_mean[g,1] <- rnorm(1, 0, cluster_var[j])
      } else {
        cluster_mean[g,1] <- rnorm(1, 0, sqrt(3))
      }
    }
    
    full_data$cluster_mean <- rep(cluster_mean, each = n_units)
    
    
    # random mean for each cluster diff between trt and ctrl
    noise <- matrix(NA, nrow = n_clusters[j]*n_units, ncol = 1)
    for (g in 1:nrow(full_data)){
      if (full_data$treatment[g] == 0){
        noise[g,1] <- rnorm(1, 0, 1)
      } else {
        noise[g,1] <- rnorm(1, 0, sqrt(3))
      }
    }
    
    full_data$noise <- as.vector(noise)
    ...
    if (j != 1 | m != 1){
      ...
      small_model_trt_1 <- brm(outcomes ~ race + (1|cluster), 
                             data = full_data_trt, family = "gaussian", prior = priorset,
      ...
      small_model_ctr_1 <- brm(outcomes ~ race + (1|cluster), 
                             data = full_data_ctr, family = "gaussian", prior = priorset,
                             warmup = 500, iter = 3000, chains = 1,
                             control = list(adapt_delta = 0.9), seed = seeds[((j-1)*3*n_sims + m + 0*n_sims)])
      ...
      large_model_trt_1 <- brm(outcomes ~ race + proportion + exp(continuous_pred) + continuous_pred + 
                               cont_ave + (1|cluster), 
                             data = full_data_trt, family = "gaussian", prior = priorset,
                             warmup = 500, iter = 3000, chains = 1,
                             control = list(adapt_delta = 0.9), seed = seeds[((j-1)*3*n_sims + m + 0*n_sims)])
      ...
      teerenstra_trt_1 <- brm(outcomes ~ (1|cluster), 
                            data = full_data_trt, family = "gaussian", prior = priorset2,
                            warmup = 500, iter = 3000, chains = 1,
                            control = list(adapt_delta = 0.9), seed = seeds[((j-1)*3*n_sims + m + 0*n_sims)])
      
      teerenstra_ctr_1 <- brm(outcomes ~ (1|cluster), 
                            data = full_data_ctr, family = "gaussian", prior = priorset2,
                            warmup = 500, iter = 3000, chains = 1,
                            control = list(adapt_delta = 0.9), seed = seeds[((j-1)*3*n_sims + m + 0*n_sims)])
    ...
    gee_true_trt <- tryCatch(geeglm(outcomes ~ race + proportion + exp(continuous_pred) + continuous_pred + cont_ave, id = cluster, data = full_data_trt, family = gaussian, corstr = "exchangeable"), error = function(e) NULL)
    
    gee_true_ctr <- tryCatch(geeglm(outcomes ~ race + proportion + exp(continuous_pred) + cont_ave, id = cluster, data = full_data_ctr, family = gaussian, corstr = "exchangeable"), error = function(e) NULL)
    
    gee_under_trt <- tryCatch(geeglm(outcomes ~ race, id = cluster, data = full_data_trt, family = gaussian, corstr = "exchangeable"), error = function(e) NULL)
    
    gee_under_ctr <- tryCatch(geeglm(outcomes ~ race, id = cluster, data = full_data_ctr, family = gaussian, corstr = "exchangeable"), error = function(e) NULL)
  ...
  crt_updated_OCT28_1_X4_con_DIFFTRT_SEP[index, 1] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,2] - 1.96*treatment_diff[,3]) & 
                                                treatment_diff[,1] <= (treatment_diff[,2] + 1.96*treatment_diff[,3]), 1, 0)) / nrow(treatment_diff)
  
  crt_updated_OCT28_1_X4_con_DIFFTRT_SEP[index, 2] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,6] - 1.96*treatment_diff[,7]) & 
                                                treatment_diff[,1] <= (treatment_diff[,6] + 1.96*treatment_diff[,7]), 1, 0)) / nrow(treatment_diff)
  
  crt_updated_OCT28_1_X4_con_DIFFTRT_SEP[index, 3] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,4] - 1.96*treatment_diff[,5]) & 
                                                treatment_diff[,1] <= (treatment_diff[,4] + 1.96*treatment_diff[,5]), 1, 0)) / nrow(treatment_diff)
  
  crt_updated_OCT28_1_X4_con_DIFFTRT_SEP[index, 4] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,8] - 1.96*treatment_diff[,9]) & 
                                                treatment_diff[,1] <= (treatment_diff[,8] + 1.96*treatment_diff[,9]), 1, 0)) / nrow(treatment_diff)
  
  crt_updated_OCT28_1_X4_con_DIFFTRT_SEP[index, 5] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,10] - 1.96*treatment_diff[,11]) & 
                                                treatment_diff[,1] <= (treatment_diff[,10] + 1.96*treatment_diff[,11]), 1, 0)) / nrow(treatment_diff)
  
  crt_updated_OCT28_1_X4_con_DIFFTRT_SEP[index, 6] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,12] - 1.96*treatment_diff[,13]) & 
                                                treatment_diff[,1] <= (treatment_diff[,12] + 1.96*treatment_diff[,13]), 1, 0)) / nrow(treatment_diff)
  
  crt_updated_OCT28_1_X4_con_DIFFTRT_SEP[index, 7] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,14] - 1.96*treatment_diff[,15]) & 
                                                treatment_diff[,1] <= (treatment_diff[,14] + 1.96*treatment_diff[,15]), 1, 0)) / nrow(treatment_diff)
  
  # MSE of every model
  crt_updated_OCT28_1_X4_con_DIFFTRT_SEP[index, 8] <- (sum(((treatment_diff[,2] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_updated_OCT28_1_X4_con_DIFFTRT_SEP[index, 9] <- (sum(((treatment_diff[,6] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_updated_OCT28_1_X4_con_DIFFTRT_SEP[index, 10] <- (sum(((treatment_diff[,4] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_updated_OCT28_1_X4_con_DIFFTRT_SEP[index, 11] <- (sum(((treatment_diff[,8] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_updated_OCT28_1_X4_con_DIFFTRT_SEP[index, 12] <- (sum(((treatment_diff[,10] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_updated_OCT28_1_X4_con_DIFFTRT_SEP[index, 13] <- (sum(((treatment_diff[,12] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_updated_OCT28_1_X4_con_DIFFTRT_SEP[index, 14] <- (sum(((treatment_diff[,14] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  # summed absolute bias of every model
  crt_updated_OCT28_1_X4_con_DIFFTRT_SEP[index, 15] <- mean(abs(treatment_diff[,1] - treatment_diff[,2]))
  
  crt_updated_OCT28_1_X4_con_DIFFTRT_SEP[index, 16] <- mean(abs(treatment_diff[,1] - treatment_diff[,6]))
  
  crt_updated_OCT28_1_X4_con_DIFFTRT_SEP[index, 17] <- mean(abs(treatment_diff[,1] - treatment_diff[,4]))
  
  crt_updated_OCT28_1_X4_con_DIFFTRT_SEP[index, 18] <- mean(abs(treatment_diff[,1] - treatment_diff[,8]))
  
  crt_updated_OCT28_1_X4_con_DIFFTRT_SEP[index, 19] <- mean(abs(treatment_diff[,1] - treatment_diff[,10]))
  
  crt_updated_OCT28_1_X4_con_DIFFTRT_SEP[index, 20] <- mean(abs(treatment_diff[,1] - treatment_diff[,12]))
  
  crt_updated_OCT28_1_X4_con_DIFFTRT_SEP[index, 21] <- mean(abs(treatment_diff[,1] - treatment_diff[,14]))
  
  # Average Interval width
  
  crt_updated_OCT28_1_X4_con_DIFFTRT_SEP[index, 22] <- mean(2*1.96*treatment_diff[,3])
  
  crt_updated_OCT28_1_X4_con_DIFFTRT_SEP[index, 23] <- mean(2*1.96*treatment_diff[,7])
  
  crt_updated_OCT28_1_X4_con_DIFFTRT_SEP[index, 24] <- mean(2*1.96*treatment_diff[,5])
  
  crt_updated_OCT28_1_X4_con_DIFFTRT_SEP[index, 25] <- mean(2*1.96*treatment_diff[,9])
  
  crt_updated_OCT28_1_X4_con_DIFFTRT_SEP[index, 26] <- mean(2*1.96*treatment_diff[,11])
  
  crt_updated_OCT28_1_X4_con_DIFFTRT_SEP[index, 27] <- mean(2*1.96*treatment_diff[,13])
  
  crt_updated_OCT28_1_X4_con_DIFFTRT_SEP[index, 28] <- mean(2*1.96*treatment_diff[,15])
  ...
  save(crt_updated_OCT28_1_X4_con_DIFFTRT_SEP, file = "crt_updated_OCT28_1_X4_con_DIFFTRT_SEP.Rdata")
