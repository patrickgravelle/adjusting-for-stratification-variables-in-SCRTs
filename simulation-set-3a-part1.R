...
crt_updated_OCT28_1_con <- matrix(NA, nrow = 243, ncol = 28)
...
# crt_updated_OCT28_1_con should be fine because index <- index + 1 should be fine through the loop
...
####################################################################################
####################################################################################
#######      List of what we'll need to change      ####### 


# we need to double the number of rows in the data set
# OR WE COULD DOUBLE THE NUMBER OF R SCRIPTS AND HAVE A SINGLE INTERACTION EFFECT FOR EACH - cuts run time in half
# Lets double then we can just copy all the R scripts and change the interaction effect
# could use the above to rerun with no interaction 

# DONE # Remove proportion^4 to be just proportion^4
# DONE # add the vector for the interaction effect
# DONE # need to add the continuous predictor to the Design 1 and 3 style simulations - but don't change strata!!!!
# DONE # #### NEED TO NOTE IN THE PAPER THIS WILL BE A NORMAL(1,1) NOT (0,1) LIKE DESIGNS 2 AND 4
# DONE # add the interaction term into the true outcomes generation
# DONE # add the interaction term to the TRUE MODELS 
# DONE # add the continuous predictor to the true models, the small model, strat model, and the kalbfleisch model
# DONE # #### within this we need to know how to within GEE model
# DONE # #### THEN we need to make sure that EMMEANS averages over the treatment properly in the interaction model
# DONE # Also I want the proportion to not be to the 4th degree because it's linear in the true outcome
# DONE # add treatment interaction to the true effect

# DONE # change the distribution of Xij to N(1,1)

# DONE # ### most important
# DONE # have to figure out the true treatment effect to compare results against
# DONE # then make sure to input the true treatment effect

# DONE # change the names of all the datasets
# duplicate the files to run the third level of the interaction effect
...
trt_int_effect <- rep(1,81)


seeds <- (1:(length(cluster_var)*n_sims*3) + (length(cluster_var)*n_sims*3)*39)
...
for (j in 1:length(cluster_var)){
  ...
  for (m in 1:n_sims){
    ...
    full_data$continuous_pred <- rnorm(n_clusters[j]*n_units, 1, 1)
    ...
    full_data$outcomes <- full_data$cluster_mean +
      ...
      full_data$noise + full_data$race + full_data$continuous_pred +
      (trt_int_effect[j])*full_data$treatment*full_data$continuous_pred
    ...
    if (j != 1 | m != 1){
      ...
      # Now we define our models
      small_model_1 <- brm(outcomes ~ treatment + continuous_pred + race + (1|cluster), 
      ...
      strat_model_1 <- brm(outcomes ~ treatment + continuous_pred + race + strata + (1|cluster), 
                         data = full_data, family = "gaussian", prior = priorset,
                         warmup = 500, iter = 3000, chains = 1,
                         control = list(adapt_delta = 0.9), seed = seeds[((j-1)*3*n_sims + m + 0*n_sims)])
      
      large_model_1 <- brm(outcomes ~ treatment + continuous_pred + race + proportion + treatment*continuous_pred 
                         + (1|cluster), 
                         data = full_data, family = "gaussian", prior = priorset,
                         warmup = 500, iter = 3000, chains = 1,
                         control = list(adapt_delta = 0.9), seed = seeds[((j-1)*3*n_sims + m + 0*n_sims)])
      
      kalbfleisch_1 <- brm(outcomes ~ treatment + continuous_pred + proportion + within + (1|cluster), 
                         data = full_data, family = "gaussian", prior = priorset,
                         warmup = 500, iter = 3000, chains = 1,
                         control = list(adapt_delta = 0.9), seed = seeds[((j-1)*3*n_sims + m + 0*n_sims)])
    ...
    gee_true <- geeglm(outcomes ~ factor(treatment) + continuous_pred + race + proportion + factor(treatment)*continuous_pred, id = cluster, data = full_data, family = gaussian, corstr = "exchangeable")
  ...
  treatment_diff[,1] <- rep(treat_effect[j] + trt_int_effect[j], n_sims)
  ...
  crt_updated_OCT28_1_con[index, 1] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,2] - 1.96*treatment_diff[,3]) & 
                                             treatment_diff[,1] <= (treatment_diff[,2] + 1.96*treatment_diff[,3]), 1, 0)) / nrow(treatment_diff)
  
  crt_updated_OCT28_1_con[index, 2] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,6] - 1.96*treatment_diff[,7]) & 
                                             treatment_diff[,1] <= (treatment_diff[,6] + 1.96*treatment_diff[,7]), 1, 0)) / nrow(treatment_diff)
  
  crt_updated_OCT28_1_con[index, 3] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,4] - 1.96*treatment_diff[,5]) & 
                                             treatment_diff[,1] <= (treatment_diff[,4] + 1.96*treatment_diff[,5]), 1, 0)) / nrow(treatment_diff)
  
  crt_updated_OCT28_1_con[index, 4] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,8] - 1.96*treatment_diff[,9]) & 
                                             treatment_diff[,1] <= (treatment_diff[,8] + 1.96*treatment_diff[,9]), 1, 0)) / nrow(treatment_diff)
  
  crt_updated_OCT28_1_con[index, 5] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,10] - 1.96*treatment_diff[,11]) & 
                                             treatment_diff[,1] <= (treatment_diff[,10] + 1.96*treatment_diff[,11]), 1, 0)) / nrow(treatment_diff)
  
  crt_updated_OCT28_1_con[index, 6] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,12] - 1.96*treatment_diff[,13]) & 
                                             treatment_diff[,1] <= (treatment_diff[,12] + 1.96*treatment_diff[,13]), 1, 0)) / nrow(treatment_diff)
  
  crt_updated_OCT28_1_con[index, 7] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,14] - 1.96*treatment_diff[,15]) & 
                                             treatment_diff[,1] <= (treatment_diff[,14] + 1.96*treatment_diff[,15]), 1, 0)) / nrow(treatment_diff)
  
  # MSE of every model
  crt_updated_OCT28_1_con[index, 8] <- (sum(((treatment_diff[,2] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_updated_OCT28_1_con[index, 9] <- (sum(((treatment_diff[,6] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_updated_OCT28_1_con[index, 10] <- (sum(((treatment_diff[,4] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_updated_OCT28_1_con[index, 11] <- (sum(((treatment_diff[,8] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_updated_OCT28_1_con[index, 12] <- (sum(((treatment_diff[,10] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_updated_OCT28_1_con[index, 13] <- (sum(((treatment_diff[,12] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_updated_OCT28_1_con[index, 14] <- (sum(((treatment_diff[,14] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  # summed absolute bias of every model
  crt_updated_OCT28_1_con[index, 15] <- mean(abs(treatment_diff[,1] - treatment_diff[,2]))
  
  crt_updated_OCT28_1_con[index, 16] <- mean(abs(treatment_diff[,1] - treatment_diff[,6]))
  
  crt_updated_OCT28_1_con[index, 17] <- mean(abs(treatment_diff[,1] - treatment_diff[,4]))
  
  crt_updated_OCT28_1_con[index, 18] <- mean(abs(treatment_diff[,1] - treatment_diff[,8]))
  
  crt_updated_OCT28_1_con[index, 19] <- mean(abs(treatment_diff[,1] - treatment_diff[,10]))
  
  crt_updated_OCT28_1_con[index, 20] <- mean(abs(treatment_diff[,1] - treatment_diff[,12]))
  
  crt_updated_OCT28_1_con[index, 21] <- mean(abs(treatment_diff[,1] - treatment_diff[,14]))
  
  # Average Interval width
  
  crt_updated_OCT28_1_con[index, 22] <- mean(2*1.96*treatment_diff[,3])
  
  crt_updated_OCT28_1_con[index, 23] <- mean(2*1.96*treatment_diff[,7])
  
  crt_updated_OCT28_1_con[index, 24] <- mean(2*1.96*treatment_diff[,5])
  
  crt_updated_OCT28_1_con[index, 25] <- mean(2*1.96*treatment_diff[,9])
  
  crt_updated_OCT28_1_con[index, 26] <- mean(2*1.96*treatment_diff[,11])
  
  crt_updated_OCT28_1_con[index, 27] <- mean(2*1.96*treatment_diff[,13])
  
  crt_updated_OCT28_1_con[index, 28] <- mean(2*1.96*treatment_diff[,15])
  ...
  save(crt_updated_OCT28_1_con, file = "crt_updated_OCT28_1_con.Rdata")
