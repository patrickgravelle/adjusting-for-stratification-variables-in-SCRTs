...
crt_missp_RE_slope_design1 <- matrix(NA, nrow = 243, ncol = 28)
...
# crt_missp_RE_slope_design1 should be fine because index <- index + 1 should be fine through the loop
...
seeds <- (1:(length(cluster_var)*n_sims*3) + (length(cluster_var)*n_sims*3)*45)
...
for (j in 1:length(cluster_var)){
  ...
  for (m in 1:n_sims){
    ...
    full_data$outcomes <- full_data$cluster_mean +
      ...
      full_data$noise + full_data$race + (full_data$slope_re)*full_data$proportion
    ...
    full_data$strat_centred <- full_data$strata - mean(full_data$strata)
    ...
    if (j != 1 | m != 1){
      ...
      large_model_1 <- brm(outcomes ~ treatment + race + proportion + (1|cluster) + (prop_centred-1|cluster), 
                         data = full_data, family = "gaussian", prior = priorset,
                         warmup = 500, iter = 3000, chains = 1,
                         control = list(adapt_delta = 0.9), seed = seeds[((j-1)*3*n_sims + m + 0*n_sims)])
    ...
    gee_true <- geeglm(outcomes ~ factor(treatment) + race + proportion, id = cluster, data = full_data, family = gaussian, corstr = "exchangeable")
  ...
  crt_missp_RE_slope_design1[index, 1] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,2] - 1.96*treatment_diff[,3]) & 
                                             treatment_diff[,1] <= (treatment_diff[,2] + 1.96*treatment_diff[,3]), 1, 0)) / nrow(treatment_diff)
  
  crt_missp_RE_slope_design1[index, 2] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,6] - 1.96*treatment_diff[,7]) & 
                                             treatment_diff[,1] <= (treatment_diff[,6] + 1.96*treatment_diff[,7]), 1, 0)) / nrow(treatment_diff)
  
  crt_missp_RE_slope_design1[index, 3] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,4] - 1.96*treatment_diff[,5]) & 
                                             treatment_diff[,1] <= (treatment_diff[,4] + 1.96*treatment_diff[,5]), 1, 0)) / nrow(treatment_diff)
  
  crt_missp_RE_slope_design1[index, 4] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,8] - 1.96*treatment_diff[,9]) & 
                                             treatment_diff[,1] <= (treatment_diff[,8] + 1.96*treatment_diff[,9]), 1, 0)) / nrow(treatment_diff)
  
  crt_missp_RE_slope_design1[index, 5] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,10] - 1.96*treatment_diff[,11]) & 
                                             treatment_diff[,1] <= (treatment_diff[,10] + 1.96*treatment_diff[,11]), 1, 0)) / nrow(treatment_diff)
  
  crt_missp_RE_slope_design1[index, 6] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,12] - 1.96*treatment_diff[,13]) & 
                                             treatment_diff[,1] <= (treatment_diff[,12] + 1.96*treatment_diff[,13]), 1, 0)) / nrow(treatment_diff)
  
  crt_missp_RE_slope_design1[index, 7] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,14] - 1.96*treatment_diff[,15]) & 
                                             treatment_diff[,1] <= (treatment_diff[,14] + 1.96*treatment_diff[,15]), 1, 0)) / nrow(treatment_diff)
  
  # MSE of every model
  crt_missp_RE_slope_design1[index, 8] <- (sum(((treatment_diff[,2] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_missp_RE_slope_design1[index, 9] <- (sum(((treatment_diff[,6] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_missp_RE_slope_design1[index, 10] <- (sum(((treatment_diff[,4] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_missp_RE_slope_design1[index, 11] <- (sum(((treatment_diff[,8] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_missp_RE_slope_design1[index, 12] <- (sum(((treatment_diff[,10] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_missp_RE_slope_design1[index, 13] <- (sum(((treatment_diff[,12] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_missp_RE_slope_design1[index, 14] <- (sum(((treatment_diff[,14] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  # summed absolute bias of every model
  crt_missp_RE_slope_design1[index, 15] <- mean(abs(treatment_diff[,1] - treatment_diff[,2]))
  
  crt_missp_RE_slope_design1[index, 16] <- mean(abs(treatment_diff[,1] - treatment_diff[,6]))
  
  crt_missp_RE_slope_design1[index, 17] <- mean(abs(treatment_diff[,1] - treatment_diff[,4]))
  
  crt_missp_RE_slope_design1[index, 18] <- mean(abs(treatment_diff[,1] - treatment_diff[,8]))
  
  crt_missp_RE_slope_design1[index, 19] <- mean(abs(treatment_diff[,1] - treatment_diff[,10]))
  
  crt_missp_RE_slope_design1[index, 20] <- mean(abs(treatment_diff[,1] - treatment_diff[,12]))
  
  crt_missp_RE_slope_design1[index, 21] <- mean(abs(treatment_diff[,1] - treatment_diff[,14]))
  
  # Average Interval width
  
  crt_missp_RE_slope_design1[index, 22] <- mean(2*1.96*treatment_diff[,3])
  
  crt_missp_RE_slope_design1[index, 23] <- mean(2*1.96*treatment_diff[,7])
  
  crt_missp_RE_slope_design1[index, 24] <- mean(2*1.96*treatment_diff[,5])
  
  crt_missp_RE_slope_design1[index, 25] <- mean(2*1.96*treatment_diff[,9])
  
  crt_missp_RE_slope_design1[index, 26] <- mean(2*1.96*treatment_diff[,11])
  
  crt_missp_RE_slope_design1[index, 27] <- mean(2*1.96*treatment_diff[,13])
  
  crt_missp_RE_slope_design1[index, 28] <- mean(2*1.96*treatment_diff[,15])
  ...
  save(crt_missp_RE_slope_design1, file = "crt_missp_RE_slope_design1.Rdata")
