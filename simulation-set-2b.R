...
crt_design2_missp_RE_t_dist <- matrix(NA, nrow = 243, ncol = 28)
...
seeds <- (1:(length(cluster_var)*n_sims*3) + (length(cluster_var)*n_sims*3)*34)
...
for (j in 1:length(cluster_var)){
  ...
  crt_design2_missp_RE_t_dist[index, 1] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,2] - 1.96*treatment_diff[,3]) & 
                                                treatment_diff[,1] <= (treatment_diff[,2] + 1.96*treatment_diff[,3]), 1, 0)) / nrow(treatment_diff)
  
  crt_design2_missp_RE_t_dist[index, 2] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,6] - 1.96*treatment_diff[,7]) & 
                                                treatment_diff[,1] <= (treatment_diff[,6] + 1.96*treatment_diff[,7]), 1, 0)) / nrow(treatment_diff)
  
  crt_design2_missp_RE_t_dist[index, 3] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,4] - 1.96*treatment_diff[,5]) & 
                                                treatment_diff[,1] <= (treatment_diff[,4] + 1.96*treatment_diff[,5]), 1, 0)) / nrow(treatment_diff)
  
  crt_design2_missp_RE_t_dist[index, 4] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,8] - 1.96*treatment_diff[,9]) & 
                                                treatment_diff[,1] <= (treatment_diff[,8] + 1.96*treatment_diff[,9]), 1, 0)) / nrow(treatment_diff)
  
  crt_design2_missp_RE_t_dist[index, 5] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,10] - 1.96*treatment_diff[,11]) & 
                                                treatment_diff[,1] <= (treatment_diff[,10] + 1.96*treatment_diff[,11]), 1, 0)) / nrow(treatment_diff)
  
  crt_design2_missp_RE_t_dist[index, 6] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,12] - 1.96*treatment_diff[,13]) & 
                                                treatment_diff[,1] <= (treatment_diff[,12] + 1.96*treatment_diff[,13]), 1, 0)) / nrow(treatment_diff)
  
  crt_design2_missp_RE_t_dist[index, 7] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,14] - 1.96*treatment_diff[,15]) & 
                                                treatment_diff[,1] <= (treatment_diff[,14] + 1.96*treatment_diff[,15]), 1, 0)) / nrow(treatment_diff)
  
  # MSE of every model
  crt_design2_missp_RE_t_dist[index, 8] <- (sum(((treatment_diff[,2] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_design2_missp_RE_t_dist[index, 9] <- (sum(((treatment_diff[,6] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_design2_missp_RE_t_dist[index, 10] <- (sum(((treatment_diff[,4] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_design2_missp_RE_t_dist[index, 11] <- (sum(((treatment_diff[,8] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_design2_missp_RE_t_dist[index, 12] <- (sum(((treatment_diff[,10] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_design2_missp_RE_t_dist[index, 13] <- (sum(((treatment_diff[,12] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_design2_missp_RE_t_dist[index, 14] <- (sum(((treatment_diff[,14] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  # summed absolute bias of every model
  crt_design2_missp_RE_t_dist[index, 15] <- mean(abs(treatment_diff[,1] - treatment_diff[,2]))
  
  crt_design2_missp_RE_t_dist[index, 16] <- mean(abs(treatment_diff[,1] - treatment_diff[,6]))
  
  crt_design2_missp_RE_t_dist[index, 17] <- mean(abs(treatment_diff[,1] - treatment_diff[,4]))
  
  crt_design2_missp_RE_t_dist[index, 18] <- mean(abs(treatment_diff[,1] - treatment_diff[,8]))
  
  crt_design2_missp_RE_t_dist[index, 19] <- mean(abs(treatment_diff[,1] - treatment_diff[,10]))
  
  crt_design2_missp_RE_t_dist[index, 20] <- mean(abs(treatment_diff[,1] - treatment_diff[,12]))
  
  crt_design2_missp_RE_t_dist[index, 21] <- mean(abs(treatment_diff[,1] - treatment_diff[,14]))
  
  # Average Interval width
  
  crt_design2_missp_RE_t_dist[index, 22] <- mean(2*1.96*treatment_diff[,3])
  
  crt_design2_missp_RE_t_dist[index, 23] <- mean(2*1.96*treatment_diff[,7])
  
  crt_design2_missp_RE_t_dist[index, 24] <- mean(2*1.96*treatment_diff[,5])
  
  crt_design2_missp_RE_t_dist[index, 25] <- mean(2*1.96*treatment_diff[,9])
  
  crt_design2_missp_RE_t_dist[index, 26] <- mean(2*1.96*treatment_diff[,11])
  
  crt_design2_missp_RE_t_dist[index, 27] <- mean(2*1.96*treatment_diff[,13])
  
  crt_design2_missp_RE_t_dist[index, 28] <- mean(2*1.96*treatment_diff[,15])
  ...
  save(crt_design2_missp_RE_t_dist, file = "crt_design2_missp_RE_t_dist.Rdata")
