...
crt_design3_missp_RE_Gamma <- matrix(NA, nrow = 243, ncol = 28)
...
seeds <- (1:(length(cluster_var)*n_sims*3) + (length(cluster_var)*n_sims*3)*2)
...
for (j in 1:length(cluster_var)){
  ...
  for (m in 1:n_sims){
    ...
    # ## First model does not account for proportion or strata but adjusts for race and has random intercepts for the clusters
    # small_model <- tryCatch(glmer(outcomes ~ factor(treatment) + race + (1|cluster), data = full_data, family = binomial(link = "logit")), error = function(e) NULL) 
    # 
    # ## Second model accounts for the strata in addition to race and random intercepts for clusters
    # strat_model <- tryCatch(glmer(outcomes ~  factor(treatment) + race + strata + (1|cluster), data = full_data, family = binomial(link = "logit")), error = function(e) NULL) 
    # 
    # ## Third model (the true model) accounts for proportion, race, and random intercepts for clusters
    # large_model <- tryCatch(glmer(outcomes ~ factor(treatment) + race + proportion + (1|cluster), data = full_data, family = binomial(link = "logit")), error = function(e) NULL) 
    # 
    # ## Literature models
    # kalbfleisch <- tryCatch(glmer(outcomes ~ factor(treatment) + proportion + within + (1|cluster), data = full_data, family = binomial(link = "logit")), error = function(e) NULL) 
    # 
    # teerenstra <- tryCatch(glmer(outcomes ~ factor(treatment) + (1|cluster), data = full_data, family = binomial(link = "logit")), error = function(e) NULL) 
    
    gee_true <- tryCatch(geeglm(outcomes ~ factor(treatment) + race + proportion, id = cluster, data = full_data, family = binomial, corstr = "exchangeable"), error = function(e) NULL) 
  ...
  crt_design3_missp_RE_Gamma[index, 1] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,2] - 1.96*treatment_diff[,3]) & 
                                         treatment_diff[,1] <= (treatment_diff[,2] + 1.96*treatment_diff[,3]), 1, 0)) / nrow(treatment_diff)
  
  crt_design3_missp_RE_Gamma[index, 2] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,6] - 1.96*treatment_diff[,7]) & 
                                         treatment_diff[,1] <= (treatment_diff[,6] + 1.96*treatment_diff[,7]), 1, 0)) / nrow(treatment_diff)
  
  crt_design3_missp_RE_Gamma[index, 3] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,4] - 1.96*treatment_diff[,5]) & 
                                         treatment_diff[,1] <= (treatment_diff[,4] + 1.96*treatment_diff[,5]), 1, 0)) / nrow(treatment_diff)
  
  crt_design3_missp_RE_Gamma[index, 4] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,8] - 1.96*treatment_diff[,9]) & 
                                         treatment_diff[,1] <= (treatment_diff[,8] + 1.96*treatment_diff[,9]), 1, 0)) / nrow(treatment_diff)
  
  crt_design3_missp_RE_Gamma[index, 5] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,10] - 1.96*treatment_diff[,11]) & 
                                         treatment_diff[,1] <= (treatment_diff[,10] + 1.96*treatment_diff[,11]), 1, 0)) / nrow(treatment_diff)
  
  crt_design3_missp_RE_Gamma[index, 6] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,12] - 1.96*treatment_diff[,13]) & 
                                         treatment_diff[,1] <= (treatment_diff[,12] + 1.96*treatment_diff[,13]), 1, 0)) / nrow(treatment_diff)
  
  crt_design3_missp_RE_Gamma[index, 7] <- sum(ifelse(treatment_diff[,1] >= (treatment_diff[,14] - 1.96*treatment_diff[,15]) & 
                                         treatment_diff[,1] <= (treatment_diff[,14] + 1.96*treatment_diff[,15]), 1, 0)) / nrow(treatment_diff)
  
  # MSE of every model
  crt_design3_missp_RE_Gamma[index, 8] <- (sum(((treatment_diff[,2] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_design3_missp_RE_Gamma[index, 9] <- (sum(((treatment_diff[,6] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_design3_missp_RE_Gamma[index, 10] <- (sum(((treatment_diff[,4] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_design3_missp_RE_Gamma[index, 11] <- (sum(((treatment_diff[,8] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_design3_missp_RE_Gamma[index, 12] <- (sum(((treatment_diff[,10] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_design3_missp_RE_Gamma[index, 13] <- (sum(((treatment_diff[,12] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  crt_design3_missp_RE_Gamma[index, 14] <- (sum(((treatment_diff[,14] - treatment_diff[,1])^2))) / nrow(treatment_diff)
  
  # summed absolute bias of every model
  crt_design3_missp_RE_Gamma[index, 15] <- mean(abs(treatment_diff[,1] - treatment_diff[,2]))
  
  crt_design3_missp_RE_Gamma[index, 16] <- mean(abs(treatment_diff[,1] - treatment_diff[,6]))
  
  crt_design3_missp_RE_Gamma[index, 17] <- mean(abs(treatment_diff[,1] - treatment_diff[,4]))
  
  crt_design3_missp_RE_Gamma[index, 18] <- mean(abs(treatment_diff[,1] - treatment_diff[,8]))
  
  crt_design3_missp_RE_Gamma[index, 19] <- mean(abs(treatment_diff[,1] - treatment_diff[,10]))
  
  crt_design3_missp_RE_Gamma[index, 20] <- mean(abs(treatment_diff[,1] - treatment_diff[,12]))
  
  crt_design3_missp_RE_Gamma[index, 21] <- mean(abs(treatment_diff[,1] - treatment_diff[,14]))
  
  # Average Interval width
  
  crt_design3_missp_RE_Gamma[index, 22] <- mean(2*1.96*treatment_diff[,3])
  
  crt_design3_missp_RE_Gamma[index, 23] <- mean(2*1.96*treatment_diff[,7])
  
  crt_design3_missp_RE_Gamma[index, 24] <- mean(2*1.96*treatment_diff[,5])
  
  crt_design3_missp_RE_Gamma[index, 25] <- mean(2*1.96*treatment_diff[,9])
  
  crt_design3_missp_RE_Gamma[index, 26] <- mean(2*1.96*treatment_diff[,11])
  
  crt_design3_missp_RE_Gamma[index, 27] <- mean(2*1.96*treatment_diff[,13])
  
  crt_design3_missp_RE_Gamma[index, 28] <- mean(2*1.96*treatment_diff[,15])
  ...
  save(crt_design3_missp_RE_Gamma, file = "crt_design3_missp_RE_Gamma.Rdata")
